...
class DoctorCollectionViewCell: UICollectionViewCell, SummaryMethod {
    ...
    public func configure(with doctor: Doctor) {
        ...
        starLabel.text = String(format: "%.1f", doctor.ratio_star)
        reviewerCountLabel.text = "(\(doctor.number_of_reviews))"
        majorNameLabel.text = doctor.majors_name
        doctorNameLabel.text = doctor.full_name
    }
