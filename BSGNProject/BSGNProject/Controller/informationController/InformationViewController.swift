...
class InformationViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet var infoNavigationBar: UINavigationBar!
    ...
    override func viewDidLoad() {
        ...
        setupNavigationBar()
        InfoService.fetchNews{ [self] result in
            switch result {
            case .success(let data):
                self.user = data
                print(self.user as Any)
                textFieldsData[0] = user?.last_name ?? ""
                textFieldsData[1] = user?.name ?? ""
                textFieldsData[2] = user?.phone ?? ""
                textFieldsData[4] = user?.contact_email ?? ""
                textFieldsData[5] = user?.address ?? ""
                textFieldsData[6] = user?.birth_date ?? ""
                textFieldsData[7] = "Thành Phố Hồ Chí Minh"
                textFieldsData[8] = "Quận 7"
                textFieldsData[9] = "Phường Phú Hữu"
                textFieldsData[10] = user?.blood_name ?? ""
                print(textFieldsData)
                self.updateUI()
