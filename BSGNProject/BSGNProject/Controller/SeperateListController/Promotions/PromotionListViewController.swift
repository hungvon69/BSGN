...
class PromotionListViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    @IBOutlet private var promotionListNavigationbar: UINavigationBar!
    ...
    override func viewDidLoad() {
        ...
        setupNavigationBar()
        PromotionService.fetchNews{ result in
            switch result {
            case .success(let data):
                if self.promotionVM == nil {
                    self.promotionVM = PromotionViewModel()
                }
                self.promotionVM?.listPromotion = data.items
                self.numberOfPromotions = data.items.count
                self.updateUI()
